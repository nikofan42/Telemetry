...
def backend(appID):
    ...
    for item in a.each():
        ...
        print(lapscomplete)
        db.child("Backend output").child(sessionName).child("Laps").child("Lap " + lapscomplete).set(data)
        raceVariables = {
            "Position": position,
        ...
        db.child("Backend output").child(sessionName).child("Race Variables").update(raceVariables)
